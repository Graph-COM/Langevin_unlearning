...
from utils import load_features, generate_gaussian, create_nested_folder, transform_array
...
class Runner():
    ...
    def train(self):
        if self.args.search_burnin:
            if self.args.dataset == 'CIFAR10_MULTI':
                ...
                burn_in_list = [1, 10, 20, 50, 100, 150, 200, 300]
            ...
            create_nested_folder('./result/LMC/'+str(self.args.dataset)+'/paint_unlearning_sigma/')
    ...
    def test_accuracy(self, w_list):
        ...
        return accuracy
    def run_unadjusted_langvin_multiclass(self, init_point, X, y, burn_in, sigma, len_list, projection = 0, batch_size = 0):
        start_time = time.time()
        w_list = unadjusted_langevin_algorithm_multiclass(init_point, self.dim_w, X, y, self.args.lam*self.n, sigma = sigma, 
                                               device = self.device, num_class = self.num_class, burn_in = burn_in, 
...
def main():
    ...
    parser.add_argument('--search_burnin', type = int, default = 0, help = 'search burn in steps')
